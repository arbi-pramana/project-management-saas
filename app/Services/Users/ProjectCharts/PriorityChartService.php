...
class PriorityChartService{
    ...
    public function executive_chart($request)
    {
        ...
        if(!empty($request->year)){
            $data['value'] = [
                Task::where('create_by',Auth::guard('users')->id())->where('start_date','like','%'.$request->year.'%')->where('priority_id',1)->get()->count(),
                Task::where('create_by',Auth::guard('users')->id())->where('start_date','like','%'.$request->year.'%')->where('priority_id',2)->get()->count(),
                Task::where('create_by',Auth::guard('users')->id())->where('start_date','like','%'.$request->year.'%')->where('priority_id',3)->get()->count(),
            ];
        } else {
            $data['value'] = [
